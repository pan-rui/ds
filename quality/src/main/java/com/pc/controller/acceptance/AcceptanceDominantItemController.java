...
public class AcceptanceDominantItemController extends BaseController {
	@Autowired
	private AcceptanceDominantItemService acceptanceDominantItemService;

	private Logger logger = LogManager.getLogger(this.getClass());
	
	@RequestMapping("/acceptanceDominantItem/add")
	@ResponseBody
	public BaseResult add(@RequestAttribute(Constants.USER_ID) String userId,
		...
		acceptanceDominantItemService.addAcceptanceDominantItem(map, ddBB);
		return new BaseResult(ReturnCode.OK);
	}

	@RequestMapping("/acceptanceDominantItem/delete")
	@ResponseBody
	public BaseResult delete(@RequestAttribute(Constants.USER_ID) String userId, @EncryptProcess ParamsVo params,
		...
		boolean b = acceptanceDominantItemService.deleteAcceptanceDominantItem(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
	...
	@RequestMapping("/acceptanceDominantItem/edit")
	@ResponseBody
	public BaseResult edit(@EncryptProcess ParamsVo params, @RequestAttribute(Constants.USER_ID) String userId,
		...
		map.put(TableConstants.UPDATE_TIME, DateUtil.convertDateTimeToString(new Date(), null));
		map.put(TableConstants.UPDATE_USER_ID, userId);
		boolean b = acceptanceDominantItemService.updateAcceptanceDominantItem(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
			return new BaseResult(ReturnCode.FAIL);
	...
	@RequestMapping("/acceptanceDominantItem/get")
	@ResponseBody
	public BaseResult get(@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		return new BaseResult(ReturnCode.OK, acceptanceDominantItemService.getAcceptanceDominantItem(map, ddBB));
	}

	@RequestMapping("/acceptanceDominantItem/getList")
	@ResponseBody
	public BaseResult getList(@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		return new BaseResult(ReturnCode.OK, acceptanceDominantItemService.getAcceptanceDominantItemList(map, ddBB));
	}

	@RequestMapping("/acceptanceDominantItem/getPage")
	@ResponseBody
	public BaseResult getPage(@RequestHeader(Constants.TENANT_ID) String tenantId, @EncryptProcess Page page,
		...
		map.put(TableConstants.IS_SEALED, 0);
		page.setParams(map);
		return new BaseResult(ReturnCode.OK, acceptanceDominantItemService.getAcceptanceDominantItemPage(page, ddBB));
	}
}
