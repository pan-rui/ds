...
public class AcceptanceAttachController extends BaseController {
	@Autowired
	private AcceptanceAttachService acceptanceAttachService;

	private Logger logger = LogManager.getLogger(this.getClass());
	
	@RequestMapping("/acceptanceAttach/add")
	@ResponseBody
	public BaseResult add(@RequestAttribute(Constants.USER_ID) String userId,
		...
		acceptanceAttachService.addAcceptanceAttach(map, ddBB);
		return new BaseResult(ReturnCode.OK);
	}

	@RequestMapping("/acceptanceAttach/delete")
	@ResponseBody
	public BaseResult delete(@RequestAttribute(Constants.USER_ID) String userId, @EncryptProcess ParamsVo params,
		...
		boolean b = acceptanceAttachService.deleteAcceptanceAttach(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
	...
	@RequestMapping("/acceptanceAttach/edit")
	@ResponseBody
	public BaseResult edit(@EncryptProcess ParamsVo params, @RequestAttribute(Constants.USER_ID) String userId,
		...
		map.put(TableConstants.UPDATE_TIME, DateUtil.convertDateTimeToString(new Date(), null));
		map.put(TableConstants.UPDATE_USER_ID, userId);
		boolean b = acceptanceAttachService.updateAcceptanceAttach(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
			return new BaseResult(ReturnCode.FAIL);
	...
	@RequestMapping("/acceptanceAttach/get")
	@ResponseBody
	public BaseResult get(@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		return new BaseResult(ReturnCode.OK, acceptanceAttachService.getAcceptanceAttach(map, ddBB));
	}

	@RequestMapping("/acceptanceAttach/getList")
	@ResponseBody
	public BaseResult getList(@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		return new BaseResult(ReturnCode.OK, acceptanceAttachService.getAcceptanceAttachList(map, ddBB));
	}

	@RequestMapping("/acceptanceAttach/getPage")
	@ResponseBody
	public BaseResult getPage(@RequestHeader(Constants.TENANT_ID) String tenantId, @EncryptProcess Page page,
		...
		map.put(TableConstants.IS_SEALED, 0);
		page.setParams(map);
		return new BaseResult(ReturnCode.OK, acceptanceAttachService.getAcceptanceAttachPage(page, ddBB));
	}
}
