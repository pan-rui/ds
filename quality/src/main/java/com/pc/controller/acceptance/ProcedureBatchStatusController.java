...
public class ProcedureBatchStatusController extends BaseController {
	@Autowired
	private ProcedureBatchStatusService procedureBatchStatusService;

	private Logger logger = LogManager.getLogger(this.getClass());
	
	@RequestMapping("/procedureBatchStatus/add")
	@ResponseBody
	public BaseResult add(@RequestAttribute(Constants.USER_ID) String userId,
		...
		procedureBatchStatusService.addProcedureBatchStatus(map, ddBB);
		return new BaseResult(ReturnCode.OK);
	}

	@RequestMapping("/procedureBatchStatus/delete")
	@ResponseBody
	public BaseResult delete(@RequestAttribute(Constants.USER_ID) String userId, @EncryptProcess ParamsVo params,
		...
		boolean b = procedureBatchStatusService.deleteProcedureBatchStatus(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
	...
	@RequestMapping("/procedureBatchStatus/edit")
	@ResponseBody
	public BaseResult edit(@EncryptProcess ParamsVo params, @RequestAttribute(Constants.USER_ID) String userId,
		...
		map.put(TableConstants.UPDATE_TIME, DateUtil.convertDateTimeToString(new Date(), null));
		map.put(TableConstants.UPDATE_USER_ID, userId);
		boolean b = procedureBatchStatusService.updateProcedureBatchStatus(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
			return new BaseResult(ReturnCode.FAIL);
	...
	@RequestMapping("/procedureBatchStatus/get")
	@ResponseBody
	public BaseResult get(@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		return new BaseResult(ReturnCode.OK, procedureBatchStatusService.getProcedureBatchStatus(map, ddBB));
	}

	@RequestMapping("/procedureBatchStatus/getList")
	@ResponseBody
	public BaseResult getList(@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		return new BaseResult(ReturnCode.OK, procedureBatchStatusService.getProcedureBatchStatusList(map, ddBB));
	}

	@RequestMapping("/procedureBatchStatus/getPage")
	@ResponseBody
	public BaseResult getPage(@RequestHeader(Constants.TENANT_ID) String tenantId, @EncryptProcess Page page,
		...
		map.put(TableConstants.IS_SEALED, 0);
		page.setParams(map);
		return new BaseResult(ReturnCode.OK, procedureBatchStatusService.getProcedureBatchStatusPage(page, ddBB));
	}
}
