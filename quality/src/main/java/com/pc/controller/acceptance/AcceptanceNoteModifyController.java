...
public class AcceptanceNoteModifyController extends BaseController {
	@Autowired
	private AcceptanceNoteModifyService acceptanceNoteModifyService;

	private Logger logger = LogManager.getLogger(this.getClass());
	
	@RequestMapping("/acceptanceNoteModify/add")
	@ResponseBody
	public BaseResult add(@RequestAttribute(Constants.USER_ID) String userId,
		...
		acceptanceNoteModifyService.addAcceptanceNoteModify(map, ddBB);
		return new BaseResult(ReturnCode.OK);
	}

	@RequestMapping("/acceptanceNoteModify/delete")
	@ResponseBody
	public BaseResult delete(@RequestAttribute(Constants.USER_ID) String userId, @EncryptProcess ParamsVo params,
		...
		boolean b = acceptanceNoteModifyService.deleteAcceptanceNoteModify(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
	...
	@RequestMapping("/acceptanceNoteModify/edit")
	@ResponseBody
	public BaseResult edit(@EncryptProcess ParamsVo params, @RequestAttribute(Constants.USER_ID) String userId,
		...
		map.put(TableConstants.UPDATE_TIME, DateUtil.convertDateTimeToString(new Date(), null));
		map.put(TableConstants.UPDATE_USER_ID, userId);
		boolean b = acceptanceNoteModifyService.updateAcceptanceNoteModify(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
			return new BaseResult(ReturnCode.FAIL);
	...
	@RequestMapping("/acceptanceNoteModify/get")
	@ResponseBody
	public BaseResult get(@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		return new BaseResult(ReturnCode.OK, acceptanceNoteModifyService.getAcceptanceNoteModify(map, ddBB));
	}

	@RequestMapping("/acceptanceNoteModify/getList")
	@ResponseBody
	public BaseResult getList(@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		return new BaseResult(ReturnCode.OK, acceptanceNoteModifyService.getAcceptanceNoteModifyList(map, ddBB));
	}

	@RequestMapping("/acceptanceNoteModify/getPage")
	@ResponseBody
	public BaseResult getPage(@RequestHeader(Constants.TENANT_ID) String tenantId, @EncryptProcess Page page,
		...
		map.put(TableConstants.IS_SEALED, 0);
		page.setParams(map);
		return new BaseResult(ReturnCode.OK, acceptanceNoteModifyService.getAcceptanceNoteModifyPage(page, ddBB));
	}
}
