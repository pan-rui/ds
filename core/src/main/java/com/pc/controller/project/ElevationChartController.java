...
import com.pc.service.project.impl.ElevationChartService;
...
public class ElevationChartController extends BaseController {
	@Autowired
	private ElevationChartService elevationChartService;

	private Logger logger = LogManager.getLogger(this.getClass());
	
	@RequestMapping("/elevationChart/add")
	@ResponseBody
	public BaseResult add(@RequestAttribute(Constants.USER_ID) String userId,
		...
		elevationChartService.addElevationChart(map, ddBB);
		return new BaseResult(ReturnCode.OK);
	}

	@RequestMapping("/elevationChart/delete")
	@ResponseBody
	public BaseResult delete(@RequestAttribute(Constants.USER_ID) String userId, @EncryptProcess ParamsVo params,
		...
		boolean b = elevationChartService.deleteElevationChart(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
	...
	@RequestMapping("/elevationChart/edit")
	@ResponseBody
	public BaseResult edit(@EncryptProcess ParamsVo params, @RequestAttribute(Constants.USER_ID) String userId,
		...
		map.put(TableConstants.UPDATE_TIME, DateUtil.convertDateTimeToString(new Date(), null));
		map.put(TableConstants.UPDATE_USER_ID, userId);
		boolean b = elevationChartService.updateElevationChart(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
			return new BaseResult(ReturnCode.FAIL);
	...
	@RequestMapping("/elevationChart/get")
	@ResponseBody
	public BaseResult get(@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		return new BaseResult(ReturnCode.OK, elevationChartService.getElevationChart(map, ddBB));
	}

	@RequestMapping("/elevationChart/getList")
	@ResponseBody
	public BaseResult getList(@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		return new BaseResult(ReturnCode.OK, elevationChartService.getElevationChartList(map, ddBB));
	}

	@RequestMapping("/elevationChart/getPage")
	@ResponseBody
	public BaseResult getPage(@RequestHeader(Constants.TENANT_ID) String tenantId, @EncryptProcess Page page,
		...
		map.put(TableConstants.IS_SEALED, 0);
		page.setParams(map);
		return new BaseResult(ReturnCode.OK, elevationChartService.getElevationChartPage(page, ddBB));
	}
}
