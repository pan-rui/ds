...
public class FloorTypeBatchNoteController extends BaseController {
	@Autowired
	private FloorTypeBatchNoteService floorTypeBatchNoteService;

	private Logger logger = LogManager.getLogger(this.getClass());
	
	@RequestMapping("/floorTypeBatchNote/add")
	@ResponseBody
	public BaseResult add(@RequestAttribute(Constants.USER_ID) String userId,
		...
		floorTypeBatchNoteService.addFloorTypeBatchNote(map, ddBB);
		return new BaseResult(ReturnCode.OK);
	}

	@RequestMapping("/floorTypeBatchNote/delete")
	@ResponseBody
	public BaseResult delete(@RequestAttribute(Constants.USER_ID) String userId, @EncryptProcess ParamsVo params,
		...
		boolean b = floorTypeBatchNoteService.deleteFloorTypeBatchNote(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
	...
	@RequestMapping("/floorTypeBatchNote/edit")
	@ResponseBody
	public BaseResult edit(@EncryptProcess ParamsVo params, @RequestAttribute(Constants.USER_ID) String userId,
		...
		map.put(TableConstants.UPDATE_TIME, DateUtil.convertDateTimeToString(new Date(), null));
		map.put(TableConstants.UPDATE_USER_ID, userId);
		boolean b = floorTypeBatchNoteService.updateFloorTypeBatchNote(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
			return new BaseResult(ReturnCode.FAIL);
	...
	@RequestMapping("/floorTypeBatchNote/get")
	@ResponseBody
	public BaseResult get(@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		return new BaseResult(ReturnCode.OK, floorTypeBatchNoteService.getFloorTypeBatchNote(map, ddBB));
	}

	@RequestMapping("/floorTypeBatchNote/getList")
	@ResponseBody
	public BaseResult getList(@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		return new BaseResult(ReturnCode.OK, floorTypeBatchNoteService.getFloorTypeBatchNoteList(map, ddBB));
	}

	@RequestMapping("/floorTypeBatchNote/getPage")
	@ResponseBody
	public BaseResult getPage(@RequestHeader(Constants.TENANT_ID) String tenantId, @EncryptProcess Page page,
		...
		map.put(TableConstants.IS_SEALED, 0);
		page.setParams(map);
		return new BaseResult(ReturnCode.OK, floorTypeBatchNoteService.getFloorTypeBatchNotePage(page, ddBB));
	}
}
