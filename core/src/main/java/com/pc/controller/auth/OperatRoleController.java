...
import com.pc.service.auth.OperatRoleService;
...
public class OperatRoleController extends BaseController {

	@Autowired
	private OperatRoleService operatRoleService;
	 ...
	 * @return
	 */
	@RequestMapping("/operatRole/getPage")
	@ResponseBody
	public BaseResult getPage(@RequestHeader(Constants.TENANT_ID) String tenantId, @EncryptProcess Page page,
		...
		page.setParams(map);
		return new BaseResult(ReturnCode.OK, operatRoleService.getOperatRolePage(page, ddBB));
	}

	@RequestMapping("/operatRole/getList")
	@ResponseBody
	public BaseResult getList(@EncryptProcess ParamsVo params, @RequestHeader(Constants.TENANT_ID) String tenantId,
		...
		map.put(TableConstants.IS_VALID, 0);
		map.put(TableConstants.IS_SEALED, 0);
		return new BaseResult(ReturnCode.OK, operatRoleService.getOperatRoleList(map, ddBB));
	}
	...
	public BaseResult add(@RequestAttribute(Constants.USER_ID) String userId,
		...
		map.put(TableConstants.IS_VALID, 0);
		map.put(TableConstants.IS_SEALED, 0);
		operatRoleService.addOperatRole(map, ddBB);
	 ...
	 * @return
	 */
	@RequestMapping("/operatRole/edit")
	@ResponseBody
	public BaseResult edit(@RequestAttribute(Constants.USER_ID) String userId, @EncryptProcess ParamsVo params,
		...
		map.put(TableConstants.UPDATE_TIME, DateUtil.convertDateTimeToString(new Date(), null));
		map.put(TableConstants.UPDATE_USER_ID, userId);
		boolean b = operatRoleService.updateOperatRole(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
	 ...
	 * @return
	 */
	@RequestMapping("/operatRole/delete")
	@ResponseBody
	public BaseResult delete(@RequestAttribute(Constants.USER_ID) String userId, @EncryptProcess ParamsVo params,
		...
		boolean b = operatRoleService.deleteOperatRole(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
			return new BaseResult(ReturnCode.FAIL);
	 ...
	 * @return
	 */
	@RequestMapping("/operatRole/get")
	@ResponseBody
	public BaseResult get(@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		return new BaseResult(ReturnCode.OK, operatRoleService.getOperatRole(map, ddBB));
	}
