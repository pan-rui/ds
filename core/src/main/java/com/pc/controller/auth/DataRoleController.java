...
import com.pc.service.auth.DataRoleService;
...
public class DataRoleController extends BaseController {
	...
	public BaseResult add(@RequestAttribute(Constants.USER_ID) String userId,
		...
		dataRoleService.addDataRole(map, ddBB);
		return new BaseResult(ReturnCode.OK);
	}

	@RequestMapping("/dataRole/delete")
	@ResponseBody
	public BaseResult delete(@RequestAttribute(Constants.USER_ID) String userId, @EncryptProcess ParamsVo params,
		...
		boolean b = dataRoleService.deleteDataRole(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
	...
	public BaseResult edit(@EncryptProcess ParamsVo params, @RequestAttribute(Constants.USER_ID) String userId,
		...
		map.put(TableConstants.UPDATE_TIME, DateUtil.convertDateTimeToString(new Date(), null));
		map.put(TableConstants.UPDATE_USER_ID, userId);
		boolean b = dataRoleService.updateDataRole(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
			return new BaseResult(ReturnCode.FAIL);
	...
	@RequestMapping("/dataRole/get")
	@ResponseBody
	public BaseResult get(@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		return new BaseResult(ReturnCode.OK, dataRoleService.getDataRole(map, ddBB));
	}

	@RequestMapping("/dataRole/getList")
	@ResponseBody
	public BaseResult getList(@RequestHeader(Constants.TENANT_ID) String tenantId, @RequestBody ParamsVo params,
			@RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		map.put(TableConstants.TENANT_ID, tenantId);
		map.put(TableConstants.IS_VALID, 0);
		map.put(TableConstants.IS_SEALED, 0);
		return new BaseResult(ReturnCode.OK, dataRoleService.getDataRoleList(map, ddBB));
	}

	@RequestMapping("/dataRole/getPage")
	@ResponseBody
	public BaseResult getPage(@RequestHeader(Constants.TENANT_ID) String tenantId, @EncryptProcess Page page,
		...
		map.put(TableConstants.IS_SEALED, 0);
		page.setParams(map);
		return new BaseResult(ReturnCode.OK, dataRoleService.getDataRolePage(page, ddBB));
	}
}
