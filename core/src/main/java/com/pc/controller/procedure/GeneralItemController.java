...
public class GeneralItemController extends BaseController {
	@Autowired
	private GeneralItemService generalItemService;

	private Logger logger = LogManager.getLogger(this.getClass());
	
	@RequestMapping("/generalItem/add")
	@ResponseBody
	public BaseResult add(@RequestAttribute(Constants.USER_ID) String userId,
		...
		generalItemService.addGeneralItem(map, ddBB);
		return new BaseResult(ReturnCode.OK);
	}

	@RequestMapping("/generalItem/delete")
	@ResponseBody
	public BaseResult delete(@RequestAttribute(Constants.USER_ID) String userId, @EncryptProcess ParamsVo params,
		...
		boolean b = generalItemService.deleteGeneralItem(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
	...
	@RequestMapping("/generalItem/edit")
	@ResponseBody
	public BaseResult edit(@EncryptProcess ParamsVo params, @RequestAttribute(Constants.USER_ID) String userId,
		...
		map.put(TableConstants.UPDATE_TIME, DateUtil.convertDateTimeToString(new Date(), null));
		map.put(TableConstants.UPDATE_USER_ID, userId);
		boolean b = generalItemService.updateGeneralItem(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
			return new BaseResult(ReturnCode.FAIL);
	...
	@RequestMapping("/generalItem/get")
	@ResponseBody
	public BaseResult get(@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		return new BaseResult(ReturnCode.OK, generalItemService.getGeneralItem(map, ddBB));
	}

	@RequestMapping("/generalItem/getList")
	@ResponseBody
	public BaseResult getList(@RequestHeader(Constants.TENANT_ID) String tenantId,@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		...
		map.put(TableConstants.TENANT_ID, tenantId);
		map.put(TableConstants.IS_SEALED, 0);
		return new BaseResult(ReturnCode.OK, generalItemService.getGeneralItemList(map, ddBB));
	}

	@RequestMapping("/generalItem/getPage")
	@ResponseBody
	public BaseResult getPage(@RequestHeader(Constants.TENANT_ID) String tenantId, @EncryptProcess Page page,
		...
		map.put(TableConstants.IS_SEALED, 0);
		page.setParams(map);
		return new BaseResult(ReturnCode.OK, generalItemService.getGeneralItemPage(page, ddBB));
	}
}
