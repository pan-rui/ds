...
import com.pc.service.sys.impl.AppSysSettingService;
...
public class AppSysSettingController extends BaseController {
	@Autowired
	private AppSysSettingService appSysSettingService;

	private Logger logger = LogManager.getLogger(this.getClass());
	
	@RequestMapping("/appSysSetting/add")
	@ResponseBody
	public BaseResult add(@RequestAttribute(Constants.USER_ID) String userId,
		...
		appSysSettingService.addAppSysSetting(map, ddBB);
		return new BaseResult(ReturnCode.OK);
	}

	@RequestMapping("/appSysSetting/delete")
	@ResponseBody
	public BaseResult delete(@RequestAttribute(Constants.USER_ID) String userId, @EncryptProcess ParamsVo params,
		...
		boolean b = appSysSettingService.deleteAppSysSetting(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
	...
	@RequestMapping("/appSysSetting/edit")
	@ResponseBody
	public BaseResult edit(@EncryptProcess ParamsVo params, @RequestAttribute(Constants.USER_ID) String userId,
		...
		map.put(TableConstants.UPDATE_TIME, DateUtil.convertDateTimeToString(new Date(), null));
		map.put(TableConstants.UPDATE_USER_ID, userId);
		boolean b = appSysSettingService.updateAppSysSetting(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
			return new BaseResult(ReturnCode.FAIL);
	...
	@RequestMapping("/appSysSetting/get")
	@ResponseBody
	public BaseResult get(@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		return new BaseResult(ReturnCode.OK, appSysSettingService.getAppSysSetting(map, ddBB));
	}

	@RequestMapping("/appSysSetting/getList")
	@ResponseBody
	public BaseResult getList(@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		return new BaseResult(ReturnCode.OK, appSysSettingService.getAppSysSettingList(map, ddBB));
	}

	@RequestMapping("/appSysSetting/getPage")
	@ResponseBody
	public BaseResult getPage(@RequestHeader(Constants.TENANT_ID) String tenantId, @EncryptProcess Page page,
		...
		map.put(TableConstants.IS_SEALED, 0);
		page.setParams(map);
		return new BaseResult(ReturnCode.OK, appSysSettingService.getAppSysSettingPage(page, ddBB));
	}
}
