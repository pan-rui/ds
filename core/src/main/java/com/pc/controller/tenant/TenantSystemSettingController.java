...
public class TenantSystemSettingController extends BaseController {
	@Autowired
	private TenantSystemSettingService tenantSystemSettingService;

	private Logger logger = LogManager.getLogger(this.getClass());
	
	@RequestMapping("/tenantSystemSetting/add")
	@ResponseBody
	public BaseResult add(@RequestAttribute(Constants.USER_ID) String userId,
		...
		tenantSystemSettingService.addTenantSystemSetting(map, ddBB);
		return new BaseResult(ReturnCode.OK);
	}

	@RequestMapping("/tenantSystemSetting/delete")
	@ResponseBody
	public BaseResult delete(@RequestAttribute(Constants.USER_ID) String userId, @EncryptProcess ParamsVo params,
		...
		boolean b = tenantSystemSettingService.deleteTenantSystemSetting(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
	...
	@RequestMapping("/tenantSystemSetting/edit")
	@ResponseBody
	public BaseResult edit(@EncryptProcess ParamsVo params, @RequestAttribute(Constants.USER_ID) String userId,
		...
		map.put(TableConstants.UPDATE_TIME, DateUtil.convertDateTimeToString(new Date(), null));
		map.put(TableConstants.UPDATE_USER_ID, userId);
		boolean b = tenantSystemSettingService.updateTenantSystemSetting(map, ddBB);
		if (b) {
			return new BaseResult(ReturnCode.OK);
		} else {
			return new BaseResult(ReturnCode.FAIL);
	...
	@RequestMapping("/tenantSystemSetting/get")
	@ResponseBody
	public BaseResult get(@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		return new BaseResult(ReturnCode.OK, tenantSystemSettingService.getTenantSystemSetting(map, ddBB));
	}

	@RequestMapping("/tenantSystemSetting/getList")
	@ResponseBody
	public BaseResult getList(@EncryptProcess ParamsVo params, @RequestAttribute String ddBB) {
		Map<String, Object> map = new LinkedHashMap<>(params.getParams());
		return new BaseResult(ReturnCode.OK, tenantSystemSettingService.getTenantSystemSettingList(map, ddBB));
	}

	@RequestMapping("/tenantSystemSetting/getPage")
	@ResponseBody
	public BaseResult getPage(@RequestHeader(Constants.TENANT_ID) String tenantId, @EncryptProcess Page page,
		...
		map.put(TableConstants.IS_SEALED, 0);
		page.setParams(map);
		return new BaseResult(ReturnCode.OK, tenantSystemSettingService.getTenantSystemSettingPage(page, ddBB));
	}
}
