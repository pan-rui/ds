...
//		fillBaseData(params, ddBB, tenantId);
		...
		userMap.addParams("USER_NAME", params.get("TENANT_NAME")).addParams("REAL_NAME", "管理员").addParams("PHONE",tenantPhone ).addParams("PWD", Base64.encode(DigestUtils.md5Hex(tenantPhone.substring(5)).getBytes())).addParams(TableConstants.IS_SEALED, "0").addParams(TableConstants.User.REGISTER_TIME, calendar.getTime()).addParams(TableConstants.TENANT_ID, tenantId).addParams(TableConstants.UPDATE_TIME, calendar.getTime());
...
@Transactional(propagation = Propagation.NESTED,rollbackFor = Exception.class)
	public void fillBaseData(final Map<String, Object> params,String ddBB,String tenantId) {
		...
		List<Map<String,Object>> dptMapList=baseDao.queryByProsInT(ddBB + TableConstants.SEPARATE + TableConstants.DATA_PRIVILEGE_TYPE, paramsMap);
	if(!CollectionUtils.isEmpty(dptMapList)) {
		...
		List<Map<String,Object>> siMapList=baseDao.queryByProsInT(ddBB + TableConstants.SEPARATE + TableConstants.STATUS_INFO, paramsMap);
	if(!CollectionUtils.isEmpty(siMapList)) {
		...
		List<Map<String,Object>> piMapList=baseDao.queryByProsInT(ddBB + TableConstants.SEPARATE + TableConstants.POST_INFO, paramsMap);
	if(!CollectionUtils.isEmpty(piMapList)) {
	...
	public void initAuthData(final Map<String, Object> params,String ddBB,String tenantId,String roleId) {
		...
		if(!CollectionUtils.isEmpty(omMapList)) {
		...
		List<Map<String,Object>> opMapList=baseDao.queryByProsInT(ddBB + TableConstants.SEPARATE + TableConstants.OPERATE_PRIVILEGES, paramsMap);
		List<Map<String, Object>> roleRelateMapList = new ArrayList<>();
		if(!CollectionUtils.isEmpty(opMapList)) {
	...
	public void initProcedure(String ddBB,Map<String,Object> map,String userId) {
		...
		if("1".equals(tenantRole.get("roleCode"))){		//质检模块
