...
public class WeatherUtil {
	...
	public static JSONObject getWeather(String cityName) {
		String url = getChniaWeatherUrl(cityName);
		...
		int temperature=0;
		if (url != null) {
			String str = getWeatherData(url);
			JSONObject data = (JSONObject) ((JSONObject) JSON.parse(str)).getJSONObject("data").getJSONArray("forecast")
					.get(0);
			weather = data.getString("type");
			max=data.getString("high").split(" ")[1].replace("℃", "");
			min=data.getString("low").split(" ")[1].replace("℃", "");

		} else {
			url = getSinaWeatherUrl(cityName);
			String data = getWeatherData(url);
			if (data.indexOf("") >= 0) {
				weather = data.substring(data.indexOf("<status1>") + 9, data.indexOf("</status1>"));
				max= data.substring(data.indexOf("<temperature1>") + 14, data.indexOf("</temperature1>"));
				min=data.substring(data.indexOf("<temperature2>") + 14, data.indexOf("</temperature2>"));
			}
		}
		...
		if (weather.indexOf("雪") >= 0) {
			...
			weather = "雨";
		} else if (weather.indexOf("雾") >= 0 || weather.indexOf("霾") >= 0 || weather.indexOf("尘") >= 0
				|| weather.indexOf("沙") >= 0 || weather.indexOf("阴") >= 0) {
			weather = "阴";
		...
		if(max!=null&&min!=null){
			temperature=(Integer.parseInt(max)+Integer.parseInt(min))/2;
		}
	...
	private static String getChniaWeatherUrl(String cityName) {
		...
		return "http://wthrcdn.etouch.cn/weather_mini?citykey=" + cityCode;
	}

	private static String getSinaWeatherUrl(String cityName) {
		try {
			return "http://php.weather.sina.com.cn/xml.php?city=" + URLEncoder.encode(cityName, "gb2312")
