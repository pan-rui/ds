...
public class SysUserFilter extends PathMatchingFilter {
    ...
    protected boolean preHandle(ServletRequest request1, ServletResponse response1) throws Exception {
        ...
        if (!filter(uri)) {
            ...
            String tenB = com.pc.core.Constants.getCache(sessionId);
            String token =  request.getHeader(Constants.USER_TOKEN);
            Map<String, Object> userMap = StringUtils.isEmpty(token) ? null : com.pc.core.Constants.getCacheValue("token", "quality-user-" + token, ParamsMap.class);
            if (userMap != null) {
