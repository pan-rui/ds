...
//        Map<String, Object> map = (Map<String, Object>) baseDao.queryByProsInTab((String)(request.getAttribute("ddBB"))+TableConstants.SEPARATE+TableConstants.USER,ParamsMap.newMap("PHONE",phone).addParams("TENANT_ID",tenantId)).get(0);
        if (map != null) {
            String tokenStr = tokenService.getToken(phone, tenantId);
            map.put("token", tokenStr);
        }
